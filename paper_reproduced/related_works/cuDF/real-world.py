...
def measure_parsing_time(json_file_path, description, query_version):
    ...
    elif query_version == 12:
         # Make sure 'id' is a top-level column
        if 'id' not in df.columns:
            print("No 'id' field found in the top-level keys.")
            return

        # Extract all IDs as a Series
        ids = df['id']

        # Optional: convert to list (expensive if large)
        id_list = ids.to_arrow().to_pylist()

        # Print or return
        print(f"Extracted {len(id_list)} IDs.")
